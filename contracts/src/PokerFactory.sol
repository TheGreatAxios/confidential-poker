...
contract PokerFactory is Ownable {
    ...
    function createTable(
        ...
        require(available >= minReserve, InsufficientPayment(minReserve, available));

        PokerGame table = new PokerGame{value: minReserve}(
            address(this),
            CHIP_TOKEN,
            msg.sender,
            buyIn,
    ...
    function removeTable(address table) external {
        require(msg.sender == table, "Only table can remove itself");
        require(isKnownTable[table], TableNotFound(table));
        isKnownTable[table] = false;

        uint256 lastIdx = tables.length - 1;
        for (uint256 i = 0; i < tables.length; i++) {
            if (tables[i] == table) {
                if (i != lastIdx) {
                    tables[i] = tables[lastIdx];
                }
                tables.pop();
                break;
            }
