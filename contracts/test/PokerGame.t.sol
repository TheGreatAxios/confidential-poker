...
contract PokerFactoryTest is Test {
    ...
    function setUp() external {
        ...
        vm.deal(address(factory), 1_000 ether);
    ...
    function testCreateTable() external {
        address table = factory.createTable(BUY_IN, SB, BB, 6, "Table 1");
        assertTrue(factory.isKnownTable(table));
        assertEq(factory.getTableCount(), 1);
        assertEq(factory.getTable(0), table);
    }

    function testCreateTableRevertsOnUnderfundedFactory() external {
        PokerFactory emptyFactory = new PokerFactory(address(chips), CALLBACK_VALUE);
        vm.expectRevert();
        emptyFactory.createTable(BUY_IN, SB, BB, 6, "Table 1");
    }

    function testGetAllTables() external {
        factory.createTable(BUY_IN, SB, BB, 6, "T1");
        factory.createTable(BUY_IN * 2, SB * 2, BB * 2, 10, "T2");
    ...
    function testGetTablesByBuyIn() external {
        factory.createTable(BUY_IN, SB, BB, 6, "T1");
        factory.createTable(BUY_IN * 2, SB * 2, BB * 2, 10, "T2");
        factory.createTable(BUY_IN, SB, BB, 6, "T3");
